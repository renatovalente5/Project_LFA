...
public class Compiler extends SourceLanguageBaseVisitor<ST>{
	...
	@Override public ST visitNewType(SourceLanguageParser.NewTypeContext ctx) { 
		//string templates
		ST line  = new ST("symb.put(\"<key>\", new Var(\"<Type>\", <Num>, ListTipo.getTipo(\"<Type>\").units()))");
	...
	@Override public ST visitAssignDeclaration(SourceLanguageParser.AssignDeclarationContext ctx) { 
		...
		ST assign = new ST("symb.get(\"<key>\").setVal((<val>).getVal())");
		assign.add("key", key);
	...
	@Override public ST visitAssignVariable(SourceLanguageParser.AssignVariableContext ctx) { 
		ST output = new ST("symb.get(\"<key>\").setVal((<val>).getVal()); \nsymb.get(\"<key>\").setNumDen(ListTipo.getTipo(symb.get(\"<key>\").getType()).units())");
	...
	@Override public ST visitExpressionDouble(SourceLanguageParser.ExpressionDoubleContext ctx) { 
		...
		ST output = new ST("(new Var(\"Double\",<Double>))");
	...
	@Override public ST visitExpressionMulDiv(SourceLanguageParser.ExpressionMulDivContext ctx) { 
		ST varCriation2 = null;

		if (ctx.op.getText().equals("*")) {
			varCriation2 = new ST("(Counts.mult(<Expression1>,<Expression2>))");
		}else{
			varCriation2 = new ST("(Counts.div(<Expression1>,<Expression2>))");
		}
	...
	@Override public ST visitExpressionSignal(SourceLanguageParser.ExpressionSignalContext ctx) { 
		ST output = new ST("new Var((<Expression>).getType(), <Sinal>(<Expression>).getVal(), (<Expression>).getNumDen())");
	...
	@Override public ST visitExpressionAssociation(SourceLanguageParser.ExpressionAssociationContext ctx) { 
		ST output = new ST("(<Expression>)");
	...
	@Override public ST visitExpressionInteger(SourceLanguageParser.ExpressionIntegerContext ctx) { 
		ST output = new ST("(new Var(\"Integer\",<Int>))");
	...
	//inicilizacion
		...
		if (ctx.op.getText().equals("+")) {
			varCriation2 = new ST("(Counts.add(<Expression1>,<Expression2>))");
		}else{
			varCriation2 = new ST("(Counts.sub(<Expression1>,<Expression2>))");
		}
		ST e1 = (ST)visit(ctx.e1);
		ST e2 = (ST)visit(ctx.e2);
